...
{  
    ...
    {
        public static double Box_Muller_Transform()    
        {
            Random r = new Random();
            double x1, x2, z1;
            x1 = r.NextDouble();
        ...
        public static (double option_value, double standard_error) option_anti(double S, double K, double T, double r, double sigma, long N, int steps, string call_put, string multi_thread)
        {
            ...
            if (multi_thread == "no")
            {
                for (int i = 0; i < N; i++)
                {
                    ...
                    {
                        option_value[i] = Math.Exp(-r * T) * Math.Max(stock_price[i, steps - 1] - K, 0);
                        option_value_anti[i] = Math.Exp(-r * T) * (Math.Max(stock_price[i, steps - 1] - K, 0) + Math.Max(stock_price_anti[i, steps - 1] - K, 0)) / 2;
                    }

                    else                  
                    {
                        option_value[i] = Math.Exp(-r * T) * Math.Max(K - stock_price[i, steps - 1], 0);
                        option_value_anti[i] = Math.Exp(-r * T) * (Math.Max(K - stock_price[i, steps - 1], 0) + Math.Max(K - stock_price_anti[i, steps - 1], 0)) / 2;
                    }
                }
            }
            else
         ...
         public static (double option_value, double standard_error) option_anti_cv(double S, double K, double T, double r, double sigma, long N, int steps, string call_put, string multi_thread)
        {
            ...
            if (multi_thread == "no")
            {
                for (int i = 0; i < N; i++)
                {
                    ...
                    if (call_put == "call")    
                    {
                        CT[i] = 0.5 * (Math.Max(0, stock_price_1[i, steps - 1] - K) + beta1 * cv_1[i, steps - 1] + Math.Max(0, stock_price_2[i, steps - 1] - K) + beta1 * cv_2[i, steps - 1]);
                    }

                    else                  
                    {
                        CT[i] = 0.5 * (Math.Max(0, K - stock_price_1[i, steps - 1]) + beta1 * cv_1[i, steps - 1] + Math.Max(0, K - stock_price_2[i, steps - 1]) + beta1 * cv_2[i, steps - 1]);
                    }
                }
            }
            else
