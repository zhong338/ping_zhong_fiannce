...
{  
    public class AsianOption
    {
        ...
        public static (double option_value, double standard_error) option_origin(double S, double K, double T, double r, double sigma, long N, int steps, string call_put, string multi_thread)
        {
            ...
            double[] ave_stock_price = new double[N];   
            ...
            if (multi_thread == "no")
            {
                for (int i = 0; i < N; i++)
                {
                    ...
                    for (int j = 1; j < steps; j++)
                    {
                        ...
                        ave_stock_price[i] += stock_price[i, j];
                    }

                    ave_stock_price[i] /= steps;
                    if (call_put == "call")    
                    {
                        option_value[i] = Math.Exp(-r * T) * Math.Max(ave_stock_price[i] - K, 0);
                    }
                    else                 
                    {
                        option_value[i] = Math.Exp(-r * T) * Math.Max(K - ave_stock_price[i], 0);
                    }
                }
            }
            else
        ...
        public static (double option_value, double standard_error) option_cv(double S, double K, double T, double r, double sigma, long N, int steps, string call_put, string multi_thread)
        {
            ...
            if (multi_thread == "no")
            {
                for (int i = 0; i < N; i++)
                {
                    ...
                    if (call_put == "call")    
                    {
                        CT[i] = Math.Max(0, ave_stock_price[i] - K) + beta1 * cv[i, steps - 1];
                    }

                    else                  
                    {
                        CT[i] = Math.Max(0, K - ave_stock_price[i]) + beta1 * cv[i, steps - 1];
                    }
                }
            }
            else
        ...
        public static (double option_value, double standard_error) option_anti(double S, double K, double T, double r, double sigma, long N, int steps, string call_put, string multi_thread)
        {
            ...
            double[,] seq = new double[N, steps];
            double[] ave_stock_price = new double[N];
            double[] ave_stock_price_anti = new double[N];
            ...
            if (multi_thread == "no")
            {
                for (int i = 0; i < N; i++)
                {
                    ...
                    for (int j = 1; j < steps; j++)
                    {
                        ...
                        ave_stock_price[i] += stock_price[i, j];
                        ave_stock_price_anti[i] += stock_price_anti[i, j];
                    }

                    ave_stock_price[i] /= steps;
                    ave_stock_price_anti[i] /= steps;

                    if (call_put == "call")    
                    {
                        option_value[i] = Math.Exp(-r * T) * Math.Max(ave_stock_price[i] - K, 0);
                        option_value_anti[i] = Math.Exp(-r * T) * (Math.Max(ave_stock_price[i] - K, 0) + Math.Max(ave_stock_price_anti[i] - K, 0)) / 2;
                    }

                    else                  
                    {
                        option_value[i] = Math.Exp(-r * T) * Math.Max(K - ave_stock_price[i], 0);
                        option_value_anti[i] = Math.Exp(-r * T) * (Math.Max(K - ave_stock_price[i], 0) + Math.Max(K - ave_stock_price_anti[i], 0)) / 2;
                    }
                }
            }
            else
        ...
        public static (double option_value, double standard_error) option_anti_cv(double S, double K, double T, double r, double sigma, long N, int steps, string call_put, string multi_thread)
        {
            ...
            double[] ave_stock_price_1 = new double[N];
            double[] ave_stock_price_2 = new double[N];
            ...
            if (multi_thread == "no")
            {
                for (int i = 0; i < N; i++)
                {
                    stock_price_1[i, 0] = S;
                    stock_price_2[i, 0] = S;
                    ave_stock_price_1[i] = S;
                    ave_stock_price_2[i] = S;
                    ...
                    for (int j = 1; j < steps; j++)
                    {
                        ...
                        ave_stock_price_1[i] += stock_price_1[i, j];
                        ave_stock_price_2[i] += stock_price_2[i, j];
                    }
                    ave_stock_price_1[i] /= steps;
                    ave_stock_price_2[i] /= steps;


                    if (call_put == "call")    
                    {
                        CT[i] = 0.5 * (Math.Max(0, ave_stock_price_1[i] - K) + beta1 * cv_1[i, steps - 1] + Math.Max(0, ave_stock_price_2[i] - K) + beta1 * cv_2[i, steps - 1]);
                    }

                    else                  
                    {
                        CT[i] = 0.5 * (Math.Max(0, K - ave_stock_price_1[i]) + beta1 * cv_1[i, steps - 1] + Math.Max(0, K - ave_stock_price_2[i]) + beta1 * cv_2[i, steps - 1]);
                    }
                }
            }
            else
